<template>
  <a-spin :loading="loading" style="width: 100%">
    ...
    >
      <template #title>
        {{ $t('verificationDataAnalysis.EEResultDimension') }}
      </template>
      <template #extra>
...
<script lang="ts" setup>
  ...
  import { queryVerifyByEEResult } from '@/api/visualization';
  ...
  const totalRes = ref({ all: 0, charts: { age: 18 } } as any);
  const scheduleRes = ref({ all: 0, charts: { age: 18 } } as any);
  ...
  const { chartOption } = useChartOption(() => {
    return {
      ...
      title: [
        {
          text: 'EE Result Classify',
          subtext: `Total ${totalRes.value.all}`,
          left: '25%',
          textAlign: 'center',
        },
        {
          text: 'Schedule Classify',
          subtext: `Total ${scheduleRes.value.all}`,
          left: '75%',
          textAlign: 'center',
        },
      ],
      ...
      xAxis: [
        {
          type: 'value',
          max: scheduleRes.value.all as number,
          splitLine: {
            show: false,
          },
          gridIndex: 1,
      ...
      yAxis: [
        {
          gridIndex: 1,
          type: 'category',
          data: Object.keys(scheduleRes.value.charts as object),
          axisLabel: {
            interval: 0,
            rotate: 25,
          },
      ...
      series: [
        ...
        },
        {
          ...
          data: Object.values(scheduleRes.value.charts as object),
        },
      ],
    };
  });
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { data: chartData } = await queryVerifyByEEResult(
        clusterValue.value,
        weekValue.value
      );
      totalRes.value = chartData.total_res;
      scheduleRes.value = chartData.schedule_res;
    } catch (err) {
